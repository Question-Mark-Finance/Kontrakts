...
// PQuestionMark
contract PQuestionMark is ERC20('PRE-?', 'P?'), ReentrancyGuard {

    address public feeAddress;

    uint256 public salePriceE35 = 1 * (10 ** 4) * (10 ** 31); 

    uint256 public constant pQuestionMarkMaximumSupply = 200 * (10 ** 3) * (10 ** 18);
    ...
    event pQuestionMarkPurchased(address sender, uint256 maticSpent, uint256 pfarmReceived);
    ...
    function buyPQuestionMark() external payable nonReentrant {
        ...
        require(pQuestionMarkRemaining > 0, "No more pfarm remaining! Come back next time!");
        ...
        require(userPfarmTally[msg.sender] < maxQuestionMarkPurchase, "user has already purchased too much pfarm");

        uint256 originalPfarmAmount = (msg.value * salePriceE35) / 1e35;

        uint256 pQuestionMarkPurchaseAmount = originalPfarmAmount;

        if (pQuestionMarkPurchaseAmount > maxQuestionMarkPurchase)
            pQuestionMarkPurchaseAmount = maxQuestionMarkPurchase;

        if ((userPfarmTally[msg.sender] + pQuestionMarkPurchaseAmount) > maxQuestionMarkPurchase)
            pQuestionMarkPurchaseAmount = maxQuestionMarkPurchase - userPfarmTally[msg.sender];
        ...
        require(pQuestionMarkPurchaseAmount > 0, "user cannot purchase 0 pfarm");
        ...
        userPfarmTally[msg.sender] = userPfarmTally[msg.sender] + pQuestionMarkPurchaseAmount;

        uint256 maticSpent = msg.value;
        uint256 refundAmount = 0;
        if (pQuestionMarkPurchaseAmount < originalPfarmAmount) {
            ...
            maticSpent = ((pQuestionMarkPurchaseAmount * msg.value * 1e24) / originalPfarmAmount) / 1e24;
